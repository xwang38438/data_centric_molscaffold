...
class GNN(torch.nn.Module):
    def __init__(self, num_tasks, num_layer = 5, emb_dim = 300, gnn_type = 'gin', 
                 drop_ratio = 0.5, graph_pooling = "max", norm_layer = 'batch_norm'):
        '''
        ...
        if 'virtual' in gnn_type:
            self.graph_encoder = GNN_node_Virtualnode(num_layer, emb_dim, JK = 'last', drop_ratio = drop_ratio, 
                                                      residual = True, gnn_name = gnn_name, norm_layer = norm_layer)
        else:
            self.graph_encoder = GNN_node(num_layer, emb_dim, JK = 'last', drop_ratio = drop_ratio, 
                                          residual = True, gnn_name = gnn_name, norm_layer = norm_layer)
        ...
        rep_dim = emb_dim
        self.predictor = torch.nn.Sequential(torch.nn.Linear(rep_dim, 2*emb_dim), 
                                             torch.nn.BatchNorm1d(2*emb_dim), torch.nn.ReLU(), torch.nn.Dropout(), 
