...
from torch_geometric.data import Data, InMemoryDataset, collate
...
def get_negative_indices(y_true, n_sample=10):
    ...
    based on the mean absolute difference. It is used for generating negative samples for the InfoNCE loss.

    Parameters:
    y_true (torch.Tensor): The tensor containing the true values. 
    n_sample (int): The number of negative samples to generate for each sample in y_true.

    Returns:
    torch.Tensor: A tensor containing the indices of the negative samples for each sample in y_true.
    """
...
### need to check the topk_indices
            ...
            # topk_mols_dict[step+1] = topk_mols
...
#     # Handle the 'smiles' attribute
#     for data in data_list:
#         if 'smiles' not in data:
#             data['smiles'] = ''

#     # Call the original collate function
#     return collate.collate(data_list)
